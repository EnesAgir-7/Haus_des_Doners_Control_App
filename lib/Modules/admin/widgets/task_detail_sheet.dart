  ...
  Future<void> _deleteSelectedComments() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Are you sure you want to delete ${_selectedCommentIndices.length} comment(s)?',
    ...
    try {
      ...
      if (success) {
        ...
        _exitSelectionMode();
        _showSnackBar('Comments deleted successfully', isError: false);
        widget.onTaskUpdated?.call();
      } else {
        _showSnackBar('Failed to delete comments');
      }
    } catch (e) {
      _showSnackBar('Error deleting comments: ${e.toString()}');
    } finally {
              ...
              child: Row(
                children: [
                  ...
                  if (_isSelectionMode) ...[
                    ...
                    Expanded(
                      child: Text(
                        '${_selectedCommentIndices.length} selected',
                        style: const TextStyle(
                          fontSize: 16,
